...
if [ $STAGE == $STAGE_PYTHON ]; then
	...
	MVN_TEST_OPTIONS="-DskipTests -Dflink.tests.with-openssl -Dflink.tests.check-segment-multiple-free -Darchunit.freeze.store.default.allowStoreUpdate=false -Dpekko.rpc.force-invocation-serialization"
